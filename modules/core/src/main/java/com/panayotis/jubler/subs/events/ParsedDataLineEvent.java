 ...
 * has been created due to the parsing action, it might be necessary to tell the
 * subtitle-processor to create a new object for the next parsing task, such as
 * after a subtitle-detail record has been parsed and created, as this record is
 * the last item in the processing chain of a multi-attributes subtitle-event,
 * where attributes are set before the detail line (ie.
 * {@link com.panayotis.jubler.subs.loader.binary.DVDMaestro}), it is necessary
 * to tell the processor not to use the current subtitle record, but creating a
 * new one instead.</p><p>
...
public class ParsedDataLineEvent extends ActionEvent {
    ...
    /**
     * Gets the reference to the internal processor currently generated this
     * event.
     *
     * @return Reference to the current instance of processor. Null if the
