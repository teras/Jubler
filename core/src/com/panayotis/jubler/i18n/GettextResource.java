 ...
 * <P>
 * Using the GNU gettext approach, compiled message catalogs are normal Java
 * ResourceBundle classes and are thus interoperable with standard
 * ResourceBundle based code.
 * <P>
 * The main differences between the Sun ResourceBundle approach and the GNU
 * gettext approach are:
 * <UL>
 * <LI>In the Sun approach, the keys are abstract textual shortcuts. In the GNU
 * gettext approach, the keys are the English/ASCII version of the messages.
 ...
 * <CODE>\</CODE><CODE>u<VAR>nnnn</VAR></CODE> syntax. Very few editors can
 * natively display international characters in this format. In the GNU gettext
 * approach, the translation files are called
 * "<VAR>Resource</VAR>.<VAR>locale</VAR>.po" and are in the encoding the
 * translator has chosen. Many editors can be used. There are at least three GUI
 ...
 * <CODE>MissingResourceException</CODE> when no translation is found. In the
 * GNU gettext approach, the
 * <CODE>gettext</CODE> function returns the (English) message key in that case.
 * <LI>In the Sun approach, there is no support for plural handling. Even the
 * most elaborate MessageFormat strings cannot provide decent plural handling.
 * In the GNU gettext approach, we have the
 * <CODE>ngettext</CODE> function.
 * </UL>
 * <P>
 * To compile GNU gettext message catalogs into Java ResourceBundle classes, the
 * <CODE>msgfmt</CODE> program can be used.
 *
 * @author Bruno Haible
 */
public abstract class GettextResource extends ResourceBundle {
     ...
     * @return the translation of <VAR>msgid</VAR>, or <VAR>msgid</VAR> if none
     * is found
     */
     ...
     * @return the translation of <VAR>msgid</VAR> depending on <VAR>n</VAR>, or
     * <VAR>msgid</VAR> or <VAR>msgid_plural</VAR> if none is found
     */
