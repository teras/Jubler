 ...
 * When a signature, or a data pattern {@link #isSubType matched} what was
 * expected by a format loader, the loader's
 * {@link AbstractBinarySubFormat#parse parse} method will continue to capture
 * the data and convert it to subtitle-records, suitable for processing with the
 * application.
 * </p><p>
 * The data file is processed line by line, and due to the multi-patterns and
 * multi-group of patterns nature, plus the fact that the same pattern can
 * interleave in each data section, such as the example of attributes (ie.
 * Colour, Display), each data line is parsed individually by an instance of
 * {@link SubtitlePatternProcessor}. Some processors must be kept through out
 * the parsing life-cycle, such as processors that parse the attributes and the
 * subtitle-events, but some processors must be removed once their responding
 * data has been parsed, such as the processors that deals with header blocks.
 * This mechanism removes the running code as needed, and simplifies the
 * maintenance of the processors.
 * </p><p>
 ...
 * group: ("Color", "Contrast", "Display_Area") and each of the definition is
 * used for the subtitle event below, although this is not strictly true, as
 * there are attributes that could be applicale for the block of data, such as
 * 'Color' and 'Contrast'.
 * </p><p>
 ...
 * treat the data as if they are appears in blocks of related items. There are
 * two major blocks in this model,
 * <ol>
 * <li>a header, represented by {@link SonHeader}, and </li>
 * <li>a group of subtitle-events, each is represented by an instance of
 * {@link SonSubEntry}. </li>
 * </ol>
 * The header section consits of data block before the signature line, and the
 * data blocks are repeated after the <i>signature</i> line. With the current
 * model where {@link SubEntry} records are stacking up, regardless of their
 * data nature, and there is no such concept as header, reference of the header
 * record must be held in every instance of the subtitle detail record. It is
 * done so to emphasize the nature of data and reflect the data model's intents.
 * The header reference, once parsed, is held globally and the same reference is
 * copied into every subtitle-event record created.
 * </p><p>
 * The parsing mechanism uses events to allow parsing loop to be intervened by
 * listeners, and this class implements several listeners:
 * <ol>
 * <li>{@link PreParseActionEventListener} - see
 * {@link #preParseAction preParseAction}</li>
 * <li>{@link PreParsingDataLineActionEventListener} - see
 * {@link #preParsingDataLineAction preParsingDataLineAction}</li>
 * <li>{@link SubtitleRecordCreatedEventListener} - see
 * {@link #recordCreated recordCreated}</li>
 * <li>{@link ParsedDataLineEventListener} - see
 * {@link #dataLineParsed dataLineParsed}</li>
 * <li>{@link PostParseActionEventListener} - see
 * {@link #postParseAction postParseAction} </li>
 * </ol>
 * </p>
 *
...
public class DVDMaestro extends AbstractBinarySubFormat implements
    ...
    /**
     * The media option selection, such as FPS, row height and column width
     * etc...
     */
    ...
    /**
     * The list which hold the group of header processors. See also
     * {@link #definePatternList definePatternList}.
     */
    protected Vector<SubtitlePatternProcessor> headerProcessorListGroup = new Vector<SubtitlePatternProcessor>();
    /**
     * The list which hold the group of attribute processors. See also
     * {@link #definePatternList definePatternList}.
     */
    protected Vector<SubtitlePatternProcessor> attributeProcessorListGroup = new Vector<SubtitlePatternProcessor>();
    /**
     * The list which hold the group of subtitle-event line processors. See also
     * {@link #definePatternList definePatternList}.
     */
     ...
     * <pre>
     * SP_NUMBER	START		END		FILE_NAME
     * </pre> which is present in every SON subtitle file, after the header
     * section and before the subtitle-detail lines. It is put here so that it
    ...
    /**
     * Flag to signify the routine loading images to actually load the image or
     * not. As loading of images takes a lot of memory, there are actions that
     * do not need to load images, but needs to know the image files only. When
     * the flag is set to true, the image is loaded with the subtitle-events but
     * when it is set to false, no images will be loaded.
     */
     ...
     * DVD-Maestro's media option selector, and define pattern list to recognise
     * the textual content of the SON subtitle file.
     */
    public DVDMaestro() {
        setFormatOrder(3);
    ...
    /**
     * Create instances of processors and insert them into their respective
     * group lists. Also add the pre-parse and post-parse listeners.
     */
    ...
    /**
     * Calling the super init method to setup the default FPS and encoding, then
     * reset son-header and son-sub-entry references to null, ready for the next
     * run.
     */
    public void init() {
        super.init();
    ...
    /**
     * Run {@link #init} and setup the class-names for all group of processors.
     * This allows the processors to know the name of the object to create when
     * it needs to. Also set the flag to indicate if the processors group is to
     * be removed after complete the parsing of the data line or not. The header
     * group should be removed completely as the data lines for it occurs only
     * once. The attribute and detail lines are recuring, so set those groups to
     * no removable. References to the object target must be set to null, so
     * that when the data line occurs a new object can be created, but not
     * before it happens.
     *
    ...
    /**
     * <p>
     * Checks to see if the created record is an instanc of {@link SonHeader} or
     * of {@link SonSubEntry}.
     * </p><p>
     * When the record is a {@link SonHeader}, set the reference of the object
     * to be the target object of all processors that process the target-object,
     * so they won't create a new instance when processing the next data lines,
     * which belongs to the header record. The record's reference is also kept
     ...
     * reference is also used as the target-object record of all attributes and
     * subtitle detail processors, so that these processor won't create a new
     * record when it parsed the next data line, such as the next attribute that
    ...
    /**
     * After a data line has been parsed, checks to see if the processor is an
     * instance of the {@link SONSubtitleEvent}, meaning if the data line parsed
     * is a detail line. If it is, then set the processor to create a new record
     * when it see one in the next data line.
     *
     * @param e Event argument.
     */
    public void dataLineParsed(ParsedDataLineEvent e) {
        ...
        if (is_record_detail)
            processorList.setCreateNewObject(true);//end if
    }

    /**
     * Checks to see if the data-line input is a blank comment line or not, that
     * means the line only contains the symbol "#".
     *
    ...
    /**
     * Checks to see if the data-line input is a palette entry header line. This
     * is defined in {@link #pat_son_palette_entries_header
    ...
    /**
     * Checks to see if the data-line input is a header line. This is the line
     * that split the header section and the detail section, and is the
     * signature line of the SON subtitle files. It is defined in      {@link SONPatternDef#p_son_subtitle_event_header
     * p_son_subtitle_event_header}.
     *
     * @param input The textual data-line.
     * @return true if the data-line is a header signature line, false
     * otherwise.
     */
     ...
     * So that the new record created will match the expected record. Also make
     * sure that the attribute processors are not removable, as they are
     * required repeatedly.
     */
    ...
    /**
     * Before a data-line is parsed by a processor, checks to see if they are
     * ignorable. The data line is IGNORABLE if they are
     * <ol>
     ...
     * </ol>
     * When data line is the header signature line then remove the instance of
     * processor that process the palette entry, as it is no longer needed. Also
     * call {@link #resetAfterHeaderLineDetected resetAfterHeaderLineDetected}
    ...
    /**
     * The collection of {@link PostParseActionEventListener} is placed here so
     * that external components can place extra code that perform actions after
     * the images has been loaded.
     */
    ...
    /**
     * The post parsing action, loading SON images. Also added the collection of
     * {@link #postImageLoadActions} so that after the images has been loaded,
    ...
    public void postParseAction(PostParseActionEvent e) {
        ...
        //background.
        LoadSonImage imageLoader = new LoadSonImage(getJubler(), subtitle_list, sonHeader.image_directory, e.getSubtitleFile());
    ...
    /**
     * Convert the existing entries to the target record of {@link SonSubEntry}
     * type. This is to allow the toString() method to generate the correct
    ...
    public Subtitles convert(Subtitles current_subs) {
        ...
        if (is_convert)
            current_subs.convert(SonSubEntry.class, this.getClassLoader());
        return current_subs;
    }

    /**
     * Performs writing out of records to a given file. The routine will only
     * perform its action when there are records in the sutitle list. If not, it
     * will simply return, without any effects.
     *
     * If the first instance of subtitle record in the set is not of the
     * {@link SonSubEntry}, the user will have to select the media file, so that
     * the video's frame-size and frame-rate can be determined. If the user
     * chooses to ignore, the default settings are used.
     *
     ...
     * {@link com.panayotis.jubler.os.FileCommunicator#save} not to replace the
     * file-name. The current mechanism set the output file as a temporary file,
     * by default, and will replace the name of the file when this routine
     * return true. However, with this loader this will generate errors, as the
     * writing action is done in a separate thread and not in the current
     * thread, thus, when this routine returns, the thread
     * {@link WriteSonSubtitle} has just started, and the file has not event
     * been created yet. The {@link WriteSonSubtitle} will handle the file-name
     * properly when it is executed.
    ...
    public boolean produce(Subtitles given_subs, File outfile, MediaFile media) throws IOException {
        ...
        WriteSonSubtitle writer = new WriteSonSubtitle(getJubler(), convert_list, moptions, outfile, FPS, ENCODING, prefs, writeOptionPane);
        writer.start();
     ...
     * needed when processing the SON subtitle file off-line, in batch mode, and
     * do not requires any manual interventions. Output encoding is assumed to
     * be UTF-8.
     *
     * @param given_subs The list of subtitle events.
     * @param outfile The file that the list will be written to.
     * @return true if the process was carried out without errors, false
     * otherwise.
     */
     ...
     *
     * @return true if the loading of images is indeed required, false
     * otherwise.
     */
     ...
     *
     * @param loadImages true if the loading of images is indeed required, false
     * otherwise.
     */
    ...
    /**
     * Gets the reference to the list of post-image loading actions. This list
     * of actions will be performed when the loading of images is completed.
     *
     * @return The reference to a non-null list of actions to be performed after
     * the loading of images is completed.
     */
