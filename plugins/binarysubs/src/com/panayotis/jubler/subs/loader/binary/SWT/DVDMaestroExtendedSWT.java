 ...
 * with images and subtitle text, namely SWT or "SON With Text". This is a newly
 * invented subtitle format by Hoang Duy Tran to allow subtitle images and
 * OCR(ed) text to be held within the software and corrected during editing
 * sessions. The text can then be extracted to others using "Save As" function
 * from the menu - produce() - such as sub-rip SRT. Entries are NEW-LINE
 * separated.</p>
 * <p>
 * The example for index file which hold reference to images and text is shown
 * here:</p>
 * <blockquote><pre>
 * st_format	2
 ...
 * </pre>
 *
 * is used as a signature for the format. At first it looks very similar to the
 * SON file's signature, but it has an extra component, that is the
 * subtitle-text, and this is shown in the signature line. The data section
 * below the signature line can be repeated, so each subtitle event could have
...
public class DVDMaestroExtendedSWT extends DVDMaestro implements ParsedDataLineEventListener, SWTPatternDef {
    ...
    /**
     * Creates a new instance of DVDMaestroExtendedSWT by calling parent to
     * intialise, then set up the pattern list.
     *
    ...
    /**
     * Create an instance of {@link SWTSubtitleText} and add it to the list of
     * processors. This is all that is needed for this, as it inherit all other
     * processors from the parent. Also clear down the group of listener when
     * data-pasrsed event occurs as this is not needed here, also in
     * {@link DVDMaestro#dataLineParsed} a new record must be created when the
     * next data line is encountered, but it is not so here. There are still
     ...
     * signature line of the SWT subtitle files. It is defined in      {@link SWTPatternDef#p_swt_subtitle_event_header
     * pat_swt_header}.
     *
    ...
    /**
     * Cheks to see if the data line input is empty or not. If it is empty then
     * it signifies a change in the data block. Check to see if a subtitle entry
     * (detail) record has been created. If it has, then it means that an old
     * instance of {@link SWTSubEntry} has been parsed successfully, and a new
     * record must now be created to hold the next block of data.
     *
     ...
     * reset swt-header and swt-sub-entry references to null, ready for the next
     * run.
     */
    @Override
    public void init() {
    ...
    /**
     * Calls the super-class to setup the processor list, but must replace the
     * name of the target classes, so {@link SWTHeader} and {@link SWTSubEntry}
     * are used instead.
     *
    ...
    /**
     * Set all processors to use {@link SWTSubEntry} after the signature line is
     * detected.
     *
     */
    @Override
    ...
    /**
     * <p>
     * Checks to see if the created record is an instanc of {@link SWTHeader} or
     * of {@link SWTSubEntry}.
     * </p><p>
     * When the record is a {@link SWTHeader}, set the reference of the object
     ...
     * </p><p>
     * References of super class's global variables are also updated to the
     * created records.
     * </p>
     *
    ...
    /**
     * Convert the existing entries to the target record of {@link SWTSubEntry}
    ...
    public Subtitles convert(Subtitles current_subs) {
        ...
        if (is_convert)
            current_subs.convert(SWTSubEntry.class, this.getClassLoader());
        return current_subs;
    }
}
