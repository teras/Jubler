    ...
    public void writeSupFile(File outfile) {
        ...
        try {
            ...
            for (int i = 0; i < sub_len; i++) {
                ...
                String old_finished_time_s = entry.getFinishTime().toString();
                int out_time = entry.getFinishTime().getFrames((long) Time.PAL_VIDEOFRAMERATE);
                int play_duration = (out_time - in_time) / 1000;

                //debugging this part - output from project-x
//bitmap.getInTime(): 331703 bitmap.getPlayTime(): 222
//bitmap.getInTime(): 559003 bitmap.getPlayTime(): 91
//bitmap.getInTime(): 651435 bitmap.getPlayTime(): 182
//bitmap.getInTime(): 837473 bitmap.getPlayTime(): 129
//bitmap.getInTime(): 968790 bitmap.getPlayTime(): 149
//bitmap.getInTime(): 2288101 bitmap.getPlayTime(): 224
//bitmap.getInTime(): 2607521 bitmap.getPlayTime(): 263
//bitmap.getInTime(): 3326504 bitmap.getPlayTime(): 210
                
                
                Time starting_time = new Time(in_time, (long) Time.PAL_VIDEOFRAMERATE);
                Time finished_time = new Time(in_time, (long) Time.PAL_VIDEOFRAMERATE, play_duration);
                String new_starting_time_s = starting_time.toString();
                String new_finished_time_s = finished_time.toString();

                int diff = entry.getFinishTime().getMilli() - finished_time.getMilli();
                play_duration += diff;

                finished_time = new Time(in_time, (long) Time.PAL_VIDEOFRAMERATE, play_duration);
                new_finished_time_s = finished_time.toString();
                ...
                //long play_duration = (in_time - (entry.getFinishTime().getMilli() * 90)) / 10;
