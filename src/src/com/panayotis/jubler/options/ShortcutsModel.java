...
public class ShortcutsModel extends AbstractTableModel {
    ...
    private void applyItemsShortcuts(JMenu menu) {
        ...
        for (int i = 0 ; i < menu.getMenuComponentCount() ; i++) {
            ...
            if (c instanceof JMenu) {
                ...
                if (pos>=0) list.get(pos).applyShortcut(item);
            }
        }
    }
    ...
    public Object getValueAt(int row, int col){
        ...
        try {
            ...
            for (int i = 0 ; i < list.size() ; i++) {
                System.out.println(list.get(i) + " - " + deflist.get(i));
            }
            return "??";
        }
        return res;
    ...
    public void keyPressed(int keyid) {
        ...
        if (modid == -1) return; // Still invalid key
    ...
    public void loadPreferences(Properties props) {
        ...
        while (m.find()) {
            ...
            if (which>=0) list.get(which).setValues(m.group(2), m.group(3));
        }
    }
    ...
    private ArrayList<Shortcut> cloneList(ArrayList<Shortcut> oldlist, boolean only_names) {
        ...
        for (int i = 0 ; i < oldlist.size() ; i++) {
            sh = oldlist.get(i);
            if (sh!=null) {
                if (only_names) newlist.add(new Shortcut(sh.text, sh.name));
                else newlist.add(new Shortcut(sh));
            }
            else {
                newlist.add(null);
            }
    ...
    class Shortcut {
        ...
        public boolean matchKeystroke(Shortcut s) {
            ...
            if (s.key_id == KeyEvent.CHAR_UNDEFINED) return false;
            if (s.key_id != key_id) return false;
            for (int i = 0 ; i < mods.length ; i++) {
                if (s.mods[i]!=mods[i]) return false;
            }
            return true;
        }
        ...
        public void applyShortcut(JMenuItem item) {
            ...
            int mask = 0;
            if (mods[0]) mask |= KeyEvent.META_DOWN_MASK;
            if (mods[1]) mask |= KeyEvent.ALT_DOWN_MASK;
            if (mods[2]) mask |= KeyEvent.CTRL_DOWN_MASK;
            if (mods[3]) mask |= KeyEvent.SHIFT_DOWN_MASK;
